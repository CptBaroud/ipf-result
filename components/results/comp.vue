<template>
  <div
    class="z-10 flex flex-col md:flex-row justify-between gap-8 md:gap-16 font-thunder rounded-xl text-white p-2 pt-3 xl:p-4 bg-gradient-to-b xl:bg-gradient-to-r border-2"
    :class="bg" style="backdrop-filter: blur(6px)">
    <p class="font-black text-[12rem] md:text-[16rem]">
      {{ TotalKg }}<span class="uppercase font-medium text-2xl">kg</span>
    </p>
    <ul class="w-full grid grid-cols-7 grid-rows-3 gap-x-4 items-center justify-between gap-y-3 font-extrabold">
      <li class="text-4xl xs:text-5xl sm:text-6xl">
        <p>S</p>
      </li>
      <li v-for="item in filteredSquats" :key="item" class="text-4xl xs:text-5xl sm:text-6xl col-span-2"
      ...
      <li v-for="item in filteredBench" :key="item" class="text-4xl xs:text-5xl sm:text-6xl col-span-2"
        :class="item.isBest ? '' : 'opacity-[0.48]'">
        <p>{{ item.value }}</p>
      </li>
      <li class="text-4xl xs:text-5xl sm:text-6xl">
      ...
      <li v-for="item in filteredDeadlifts" :key="item" class="text-4xl xs:text-5xl sm:text-6xl col-span-2"
        :class="item.isBest ? '' : 'opacity-[0.48]'">
        <p>{{ item.value }}</p>
      </li>
    </ul>
